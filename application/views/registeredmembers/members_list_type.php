...
<h5 class="breadcrumbs-title"><?php if($type==1) { echo 'Deactivated Members'; } else if($type==2) { echo 'Association Members'; } else if($type==3) { echo 'Members Not Assigned Flats'; } else if($type==4) { echo 'Members With Multiple Flats'; } ?></h5>
<ol class="breadcrumb">
<li><a href="<?=base_url('dashboard')?>">Dashboard</a> / </li>
<li><a href="<?=base_url('registeredmembers')?>"><?=$this->p_var?></a> / </li>
<li class="active"><?php if($type==2) { echo 'Association Members'; } else if($type==3) { echo 'Members Not Assigned Flats'; } else if($type==4) { echo 'Members With Multiple Flats'; } ?></li>
</ol>
</div>
...
<div class="row height-600 mt10">
	<div class="col s12 table-cover">
		<table class="striped bordered data_list user_data">
			...
			<tbody>
				<?php
					foreach ($data as $key => $value) {
						...
						echo '</td><td>'.h($value->firstname).' '.h($value->lastname);
						echo '</td><td>';
						if(!empty($value->name)){
							echo h($value->name).' - ';
						}
						echo $value->flat_no.'</td><td>'.$value->mobile_no.'</td><td>'.h($value->email).'</td><td>'.h($value->designation).'</td>';
