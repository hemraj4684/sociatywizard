$(d).ready(function(){
	        ...
	        delete: {
	        	...
	        	callback: function(key, opt){
	        		if(confirm('Are you sure, you want to delete this folder? All the related files will be lost!')){
		                ...
		                $.ajax({
		                	type:'get',
		                	url:URL+'docsform/delete_folder/'+id,
	...
	$('#folder_prop').submit(function(e){
		...
		$.ajax({
			type:'post',
			url:URL+'docsform/update_folder',
			data:t.serialize(),
			dataType:'json',
			complete:function(){
			...
			success:function(res){
				...
				if(res.success){
					$('.u-res').html('<div class="card green"><div class="card-content success-area white-text"><i class="fa fa-thumbs-o-up"></i> Folder Successfully Updated!</div></div>')
					setTimeout(function(){
						location.reload()
					},1000)
				}
	...
	$('#new_folder_form').submit(function(e){
		...
		$.ajax({
			...
			success:function(res){
				...
				if(res.success){
					t[0].reset()
					$('.res').html('<div class="card success-area green"><div class="card-content white-text"><i class="fa fa-thumbs-o-up"></i> Folder Created Successfully!</div></div>')
					setTimeout(function(){
						location.reload()
					},1000)
				}
